...
$id=$_GET['id'];
$valid_role=$_SESSION['valid_role'];

$query23 = "SELECT * FROM event WHERE eventid='$id' ";
$exec23 =  mysqli_query($con,$query23);
$exec_set = mysqli_fetch_array($exec23);

$query232 = "SELECT * FROM joinevent WHERE eventid='$id' and userid='$userid' and iscancel!='1'";
$exec232 =  mysqli_query($con,$query232);
$exec_set2 = mysqli_fetch_array($exec232);

$query2321 = "SELECT * FROM joinevent WHERE eventid='$id' and userid!='$userid' and iscancel!='1' and eventstatus='1'";
$exec2321 =  mysqli_query($con,$query2321);
$exec_set21 = mysqli_fetch_array($exec2321);
?>
<head>
	<meta charset="utf-8">
                                ...
                                <div>
                                    ...
                                    <div class="content">
										...
										<!--	Benefits:
											...
											<br>
											Fee : <//?php //if($exec_set['eventfee']!=""){ echo $exec_set['eventfee']; } else{ echo "Free"; }?>
											<br> -->
											Location : <?php echo $exec_set['eventlocation'];?></p>
										
                                         <?php
                                        if(isset($exec_set2['userid'])){
                                        ?>
                                        	<a href="#" style="background-color:red;" class="default-button">Joined</a>

                                        <?php
                                    	}
										else if(isset($exec_set21['userid'])){
										?>
                                        	<a href="joinevent.php?id=<?php echo $exec_set['eventid'];?>" onclick="return confirm('Are you sure you want to join this event?')" class="default-button">Join</a>
                                        <?php
										}else if(($exec_set['eventstatus']=='1') && ($valid_role!='3')){
										?>
                                        	<a href="#" class="default-button" style="background-color:red;">New event</a>
                                        <?php

										}else if(($exec_set['eventstatus']=='2') && ($valid_role!='3')){
										?>
											<a class="default-button-danger" href="#" style="background-color:red;">On Going Event</a>
										<?php
										}else if(($exec_set['eventstatus']=='2') && ($valid_role=='3')){
										?>
										<a class="default-button-danger" href="#" style="background-color:red;">On Going Event</a>
